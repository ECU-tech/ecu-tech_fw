...
cp java_console_binary/ecu-tech_console.jar $CONSOLE_FOLDER
cp simulator/build/ecu-tech_simulator.exe     $CONSOLE_FOLDER
cp misc/console_launcher/ecu-tech_*.exe     $CONSOLE_FOLDER
...
cp firmware/deliver/ecu-tech.bin $FOLDER

# bootloader
[ -e firmware/deliver/ecu-tech_bl.bin ] && { cp firmware/deliver/ecu-tech_bl.bin $FOLDER ; }
# update srec
[ -e firmware/deliver/ecu-tech_update.srec ] && { cp firmware/deliver/ecu-tech_update.srec $FOLDER ; }

[ -e firmware/deliver/ecu-tech.bin ] || { echo "$SCRIPT_NAME: ecu-tech.bin not found"; exit 1; }
...
rm -rf $CONSOLE_FOLDER/ecu-tech_console.exe
