...
//        System.out.println("DFU lookup cost " + cost + "ms");
    ...
    private static String getDfuWriteCommand() throws FileNotFoundException {
        String fileName = Launcher.INPUT_FILES_PATH + File.separator + "ecu-tech.bin";
        if (fileName == null)
            throw new FileNotFoundException("File not found: ecu-tech.bin");
    ...
    static List<String> getDevicesReport() {
        ...
        try {
            ...
            while ((line = stdout.readLine()) != null) {
                ...
                if (!lowerCase.contains("stm32") && !lowerCase.contains("dfu") && !lowerCase.contains("ecu-tech"))
                    continue;
                report.add(line);
            }
            stdout.close();
