...
public enum SerialPortScanner {
    ...
    public enum SerialPortType {
        Ecu-TechEcu("ECU-TECH ECU", 20),
        Ecu-TechEcuWithOpenblt("ECU-TECH ECU w/ BL", 20),
    ...
    public static class PortResult {
        ...
        public boolean isEcu() {
            return type == SerialPortType.Ecu-TechEcu || type == SerialPortType.Ecu-TechEcuWithOpenblt;
        }
    }
    ...
    private static PortResult inspectPort(String serialPort) {
        ...
        if (isOpenblt) {
            ...
            if (isEcu) {
                boolean ecuHasOpenblt = ecu-techEcuHasOpenblt(serialPort);
                log.info("ECU-TECH ECU at " + serialPort + (ecuHasOpenblt ? " has" : " does not have") + " an OpenBLT bootloader");
                return new PortResult(serialPort, ecuHasOpenblt ? SerialPortType.Ecu-TechEcuWithOpenblt : SerialPortType.Ecu-TechEcu, signature);
            } else {
    ...
    private void findAllAvailablePorts(boolean includeSlowLookup) {
        ...
        if (includeSlowLookup) {
            for (String tcpPort : TcpConnector.getAvailablePorts()) {
                ports.add(new PortResult(tcpPort, SerialPortType.Ecu-TechEcu));
            }
        }
    ...
    public static boolean ecu-techEcuHasOpenblt(String port) {
        try (IoStream stream = BufferedSerialIoStream.openPort(port)) {
            if (stream == null) {
                return false;
            }
            ...
            byte[] response = stream.getDataBuffer().getPacket(500, "ecu-techEcuHasOpenblt");
