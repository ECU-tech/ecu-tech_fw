...
cd ../unit_tests
make -j4
if not exist build/ecu-tech_test.exe echo UNIT TEST COMPILATION FAILED
if not exist build/ecu-tech_test.exe exit -1

rem This is me using Cygwin on all my Windows devices
ls -l build/ecu-tech_test.exe
 

del triggers.txt
build\ecu-tech_test.exe
pwd

if not exist triggers.txt echo triggers.txt generation FAILED
if not exist triggers.txt exit -1

java -cp ../java_console_binary/ecu-tech_console.jar com.rusefi.TriggerImage